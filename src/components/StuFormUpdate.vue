...
import type { CommitResp } from "@/types/apis/common";
...
const updateStatus = () => {
  ...
  commonCommits(paginator, false, 2, queryInfo).then((response) => {
...
const rules = {
  ...
  domicile: [{ required: true, message: "请输入户籍所在地", trigger: "blur" }],
  home_address: [
  ...
  bachelor_course: [
    {
      required: true,
      message: "请输入原本科专业所属的国家“双一流”建设学科",
      trigger: "blur",
    },
  ],
  major: [{ required: true, message: "请输入原本科专业", trigger: "blur" }],
};
...
const submitApplyForm = () => {
  studentUpdateProfile(studentData.value)
    .then((response) => {
...
const handleAvatarSuccess = (
  ...
  if (res.code === 0) {
    ...
    // 清理旧的URL对象
    if (studentData.value.photo?.startsWith("blob:")) {
      URL.revokeObjectURL(studentData.value.photo);
    }

    // 使用服务器路径获取实际图片数据
    axios
      .get(`${useSiteInfoStore().getBaseUrl()}/student/getPhoto`, {
